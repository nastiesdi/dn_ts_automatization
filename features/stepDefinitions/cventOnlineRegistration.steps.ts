...
export = function cventSteps() {
    ...
    //Two Attendees
    this.Given(/^a user navigates to Cvent to register two attendees for an event$/, async () => {
        //For debugging in case of fails
        //await console.log(`Your autogenerated unique e-mail is 'emsautomation${uniqueIndentifier}@mailinator.com'`);
        await browser.navigate().to(browser.baseUrl);
        await event.Loaded();
    ...
    this.Given(/^a user navigates to Cvent to register himself for an event$/, async () => {
        //For debugging in case of fails
        //await console.log(`Your autogenerated unique e-mail is 'emsautomation${uniqueIndentifier}@mailinator.com'`);
        await browser.navigate().to(browser.baseUrl);
        await event.Loaded();
    ...
    //Two Attendees
    this.Then(/^enter the data in the registration form for 2 attendees$/, async () => {
        await event.RegisterTwoAttendees(uniqueIndentifier);
    });

    //Three Attendees
    this.Then(/^register an additional delegates$/, async () => {
        await event.RegisterThreeAttendees(uniqueIndentifier);
    });
