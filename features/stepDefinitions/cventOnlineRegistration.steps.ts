...
export = function cventSteps() {
    ...
    this.Then(/^verify the content of the Json$/, async () => {
        ...
        expect(Promise.resolve(JSON.parse(responseObject)[0].salesforce_request.primaryContact.aci)).to.eventually.have.string(`England`);
        ...
        expect(Promise.resolve(JSON.parse(responseObject)[0].salesforce_request.billingContact.aci)).to.eventually.have.string(`England`);
